...
flag = ProtectedFlag("flag{dummy_flag}")
```
これにより、変数を`eval("flag")`のようにそのまま評価してもフラグは返ってこず、`flag[0]`のようなアクセスか、`flag._value`のような取得が必要となる。  
...
bbhhzz
```
つまり、最初に`abcdefghijklmnopqrstuvwxyz`を送り、ルールのうち一つを記録しておき、同じルールが適用されるまで何度も試行すればよい。  
